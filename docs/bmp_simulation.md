...
After baseline pathway yields are established, both load-generation modes use the same BMP scenario engine.
...
6. applies BMP effects to the current pathway yields;
7. calculates BMP cost when cost inputs are configured;
8. updates parcel and outlet results; and
9. continues until the configured stopping condition is reached.

## Pathway-specific efficiency

For current pathway yield `Y`, treated fraction `f_t`, and sampled BMP efficiency `e`:
